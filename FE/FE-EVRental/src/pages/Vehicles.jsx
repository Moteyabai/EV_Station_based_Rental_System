        ...
        <div className="vehicles-main">
          {filteredVehicles.length === 0 ? (
            <div className="no-results">
              <p>Không tìm thấy xe máy điện phù hợp với bộ lọc đã chọn. Vui lòng thử lại với bộ lọc khác.</p>
            </div>
          ) : (
            <div className="vehicles-grid">
              {filteredVehicles.map(vehicle => (
                <div key={vehicle.id} className="vehicle-card">
                  <div className="vehicle-thumb" style={{ backgroundImage: `url(${vehicle.image})` }}></div>
                  <div className="vehicle-info">
                    <h3>{vehicle.name}</h3>
                    <p className="vehicle-short">{vehicle.short}</p>
                    <div className="vehicle-meta">
                      <span className="vehicle-brand">{vehicle.brand}</span>
                      <span className="vehicle-category">{vehicle.category === 'scooter' ? 'Xe máy điện' : vehicle.category}</span>
                    </div>
                    ...
                    <div className="vehicle-actions">
                      <Link to={`/vehicles/${vehicle.id}`} className="btn view-details">Chi tiết</Link>
                      <Link to={`/stations?vehicleId=${vehicle.id}`} className="btn rent-now">Thuê ngay</Link>
                    </div>
                  </div>
                </div>
              ))}
